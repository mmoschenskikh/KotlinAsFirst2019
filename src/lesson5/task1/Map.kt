 ...
 */
fun subtractOf(a: MutableMap<String, String>, b: Map<String, String>) = b.forEach { a.entries.remove(it) }
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean =
    word.toLowerCase().toSet().all { char -> char in chars.map { it.toLowerCase() }.toSet() }
...
fun hasAnagrams(words: List<String>): Boolean =
    words.map { word -> word.toList().groupingBy { it }.eachCount() }.toSet().size < words.size
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    list.forEachIndexed { index, it ->
        if (number - it in list.toSet() && index != list.indexOf(number - it))
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    val capacities =
        (listOf(0) + treasures.values.filter { it.first < capacity }.map { it.first } +
                List(treasures.size) { index -> (index + 1) * capacity / treasures.size }).toSet().toList().sorted()
